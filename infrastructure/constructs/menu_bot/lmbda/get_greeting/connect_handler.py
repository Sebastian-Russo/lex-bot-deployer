...
def parse_env_var(name: str, default: Optional[T] = None, required: bool = False,
                 coerce: Optional[Callable[[str], T]] = None) -> Optional[T]:
    """
...
class ConnectHandler:
    ...
    def handler(self, event: Dict[str, Any], context=None) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info("Language parameter: %s, contact language: %s", lang_param, lang_code)
            ...
            if not locale_config:
                logger.warning("No config found for language %s, available languages: %s",
                              lang, list(self.config.keys()))
...
def handler(event, context):
    ...
    """
    try:
        logger.info("LAMBDA STARTING: connect_handler with event: %s", json.dumps(event))
